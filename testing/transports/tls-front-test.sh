...
cleanup() {
	docker rm -f wsl-node wsl-up wsl-tool >/dev/null 2>&1 || true
...
docker run -d --name wsl-node --network "$NET" --ip "$NODE_IP" --cap-add NET_ADMIN \
...
sc=$(docker run --rm --net container:wsl-node "$TOOLS" \
...
scfp=$(docker run --rm --net container:wsl-node "$TOOLS" \
...
hb=$(docker run --rm --net container:wsl-node "$TOOLS" \
...
hp=$(docker run --rm --net container:wsl-node "$TOOLS" \
...
nm=$(docker run --rm --net container:wsl-node "$TOOLS" \
...
logmark=$(docker logs wsl-node 2>&1 | wc -l)
t0=$(ticks)
r1=$(docker run --rm -i --net container:wsl-node "$TOOLS" python3 - <<'PY'
import socket, time
...
if docker logs wsl-node 2>&1 | tail -n +"$logmark" | grep -q "sent no recognisable preamble within"; then
	note "R-1: undecided connection closed by the front deadline: $(docker logs wsl-node 2>&1 | tail -n +"$logmark" | grep -o 'sent no recognisable preamble within [0-9]* s')"
else
...
docker exec wsl-node sh -c 'mkdir -p /srv/decoy && head -c 8000000 /dev/zero > /srv/decoy/big.bin && printf "It works!\n" > /srv/decoy/index.html'
docker exec wsl-node sh -c "sed -i 's/      AddressPool:/      HttpsDecoyRoot: \/srv\/decoy\n      AddressPool:/' /etc/tincstack/tinc.yaml"
docker exec wsl-node tinc -c /etc/tincstack/tinc.yaml -n wsg1 reload >/dev/null 2>&1 || true
sleep 1
logmark=$(docker logs wsl-node 2>&1 | wc -l)
t0=$(ticks)
docker run --rm -i --net container:wsl-node "$TOOLS" python3 - <<'PY' >/dev/null
import socket, time
...
if docker logs wsl-node 2>&1 | tail -n +"$logmark" | grep -q "Timeout from .* during authentication"; then
...
docker run -d --name wsl-up --network "$NET" --ip "$UP_IP" "$TOOLS" python3 -c '
import socket
...
docker exec wsl-node sh -c "sed -i 's/      AddressPool:/      HttpsDecoyUpstream: $UP_IP:80\n      AddressPool:/' /etc/tincstack/tinc.yaml"
docker exec wsl-node tinc -c /etc/tincstack/tinc.yaml -n wsg1 reload >/dev/null 2>&1 || docker restart wsl-node >/dev/null
sleep 3

up=$(docker run --rm --net container:wsl-node "$TOOLS" curl -sk https://127.0.0.1:655/ || true)
...
docker exec wsl-up sh -c ': > /tmp/upstream.log'
docker run --rm --net container:wsl-node "$TOOLS" curl -sk -o /dev/null https://127.0.0.1:655/ws \
...
sleep 1
uplog=$(docker exec wsl-up cat /tmp/upstream.log 2>/dev/null || true)
if [ -z "$uplog" ]; then
...
# 10.41.9.250 has no host on the lab subnet: connect() never completes.
docker exec wsl-node sh -c "sed -i 's/HttpsDecoyUpstream: .*/HttpsDecoyUpstream: 10.41.9.250:80/' /etc/tincstack/tinc.yaml"
docker exec wsl-node tinc -c /etc/tincstack/tinc.yaml -n wsg1 reload >/dev/null 2>&1 || true
sleep 1
...
docker run --rm --net container:wsl-node "$TOOLS" curl -sk --max-time 10 -o /dev/null https://127.0.0.1:655/ >/dev/null 2>&1 &
sleep 0.5
p2=$(docker run --rm --net container:wsl-node "$TOOLS" curl -sk --max-time 10 -w '%{time_appconnect} %{time_total}' -o /dev/null https://127.0.0.1:655/ 2>/dev/null || echo "fail fail")
wait
p2hs=${p2%% *}

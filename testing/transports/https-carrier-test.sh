...
#
# Usage: [PFX=wslh] [NETBASE=10.42.9] testing/transports/https-carrier-test.sh [image]
#   PFX names the containers (<PFX>-a, -b, -cap, -mitm), the network and the
#   data directory; NETBASE is the lab /24. Override both to run next to
#   another copy of this lab on the same host.
set -e

IMG=${1:-tincstack/core:${TINCSTACK_TAG:-ws-l}}
TOOLS=nicolaka/netshoot
PFX=${PFX:-wslh}
NETBASE=${NETBASE:-10.42.9}
NET=${PFX}https
BASE=/tmp/${PFX}-https
A_IP=$NETBASE.10
B_IP=$NETBASE.11

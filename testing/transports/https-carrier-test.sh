...
#
# Usage: [LAB=prefix] [SUBNET=10.42.9] testing/transports/https-carrier-test.sh [image]
#   LAB (default wslh) prefixes every container/network name and the /tmp
...
cleanup() {
	docker rm -f "$LAB-a" "$LAB-b" "$LAB-cap" "$LAB-mitm" >/dev/null 2>&1 || true
...
setvpn() { # name vpnip
	docker exec "$LAB-$1" ip addr add "$2/24" dev wsg1 2>/dev/null || true
	docker exec "$LAB-$1" ip link set wsg1 up
}
...
docker run -d --name "$LAB-cap" --net "container:$LAB-b" --cap-add NET_RAW "$TOOLS" \
...
ping_ab=$(docker exec "$LAB-a" ping -c3 -W2 "$B_VPN" 2>&1 | tail -2)
ping_ba=$(docker exec "$LAB-b" ping -c3 -W2 "$A_VPN" 2>&1 | tail -2)
...
dca=$(docker exec "$LAB-a" tinc -c /etc/tincstack/tinc.yaml -n wsg1 dump connections 2>/dev/null || true)
dcb=$(docker exec "$LAB-b" tinc -c /etc/tincstack/tinc.yaml -n wsg1 dump connections 2>/dev/null || true)
...
pr=$(docker run --rm --net "container:$LAB-b" "$TOOLS" curl -sk https://127.0.0.1:655/ || true)
...
# $SUBNET.250 has no host: B's upstream connect never completes. Three TLS
...
docker exec "$LAB-b" sh -c "sed -i 's/      AddressPool:/      HttpsDecoyUpstream: $SUBNET.250:80\n      AddressPool:/' /etc/tincstack/tinc.yaml"
docker exec "$LAB-b" tinc -c /etc/tincstack/tinc.yaml -n wsg1 reload >/dev/null 2>&1 || true
...
while [ "$(date +%s)" -lt "$deadline" ]; do
	docker exec "$LAB-a" ping -c1 -W1 "$B_VPN" >/dev/null 2>&1 && break
	sleep 1
done
note "after B's reload A reconnected: $(docker exec "$LAB-a" tinc -c /etc/tincstack/tinc.yaml -n wsg1 dump connections 2>/dev/null | grep nodeb | grep -o 'transport [a-z]*')"
for n in a b; do
	[ "$(docker inspect -f '{{.State.Running}}' "$LAB-$n" 2>/dev/null)" = true ] || { miss "node $n is not running before the M5-1 probes"; docker logs --tail 5 "$LAB-$n" 2>&1 | sed 's/^/    /'; }
done
(for _ in 1 2 3; do docker run --rm --net "container:$LAB-b" "$TOOLS" curl -sk --max-time 10 -o /dev/null https://127.0.0.1:655/ >/dev/null 2>&1; done) &
sleep 0.3
pp=$(docker exec "$LAB-a" ping -c 12 -i 0.5 -W 3 "$B_VPN" 2>&1 | tail -2)
wait
echo "$pp" | sed 's/^/    /'
for n in a b; do
	[ "$(docker inspect -f '{{.State.Running}}' "$LAB-$n" 2>/dev/null)" = true ] || { miss "node $n died during the M5-1 probes"; docker logs --tail 8 "$LAB-$n" 2>&1 | sed 's/^/    /'; }
done
...
docker rm -f "$LAB-a" >/dev/null 2>&1 || true
unpin_a "$MITM_IP"
# The bump: its own P-256 certificate, re-encrypts towards B.
docker run -d --name "$LAB-mitm" --network "$NET" --ip "$MITM_IP" "$TOOLS" sh -c "
...
sleep 2
mitm_fp=$(docker exec "$LAB-mitm" cat /tmp/mitm.fp 2>/dev/null || true)
start a "$A_IP" "$BASE-a"
# Give A time for at least two dial attempts through the bump.
deadline=$(( $(date +%s) + 15 ))
while [ "$(date +%s)" -lt "$deadline" ]; do
	docker logs "$LAB-a" 2>&1 | grep -q "falling back to plain" && break
	sleep 1
done
pins=$(pin_of_a)
...
docker logs "$LAB-a" 2>&1 | grep -q "https: authenticated\|transport https" && miss "M5-7: A established https through the bump?!" || note "M5-7: https through the bump failed (no 101), as it must"

echo "===== M5-7: a legitimate first dial pins B's real fingerprint ====="
docker rm -f "$LAB-a" "$LAB-mitm" >/dev/null 2>&1 || true
unpin_a "$B_IP"
start a "$A_IP" "$BASE-a"
deadline=$(( $(date +%s) + 20 ))
while [ "$(date +%s)" -lt "$deadline" ]; do
	docker logs "$LAB-a" 2>&1 | grep -q "pinning TlsFingerprint" && break
	sleep 1
done
sleep 1
pins=$(pin_of_a)
...
docker logs "$LAB-a" 2>&1 | grep "SPTPS authenticated .* pinning" | head -1 | sed 's/^/    /'
setvpn a "$A_VPN"
sleep 1
docker exec "$LAB-a" ping -c2 -W2 "$B_VPN" 2>&1 | grep -q "0% packet loss" && note "M5-7: tunnel up over https after the pin" || miss "M5-7: tunnel not up after the legitimate dial"

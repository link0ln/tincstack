...
if echo "$ping_ab" | grep -q "0% packet loss"; then
	note "A -> B ping over the tunnel works"
else
	miss "A -> B ping failed"
fi
if echo "$ping_ba" | grep -q "0% packet loss"; then
	note "B -> A ping over the tunnel works"
else
...
if echo "$dca" | grep -q "transport https"; then
	note "A dump connections shows transport https"
else
	miss "A does not show transport https"
fi
if echo "$dcb" | grep -q "transport https"; then
	note "B dump connections shows transport https"
else
	miss "B does not show transport https"
fi
...
if echo "$r1" | grep -q "101"; then
	miss "forged authenticator was accepted (101)!"
else
	note "forged authenticator got the decoy (no 101)"
fi
if echo "$r2" | grep -q "101"; then
	miss "replayed authenticator was accepted (101)!"
else
	note "replayed authenticator got the decoy (no 101)"
fi
...
# shellcheck disable=SC2329  # invoked indirectly, as a command name passed to wait_for
...
l2_check() { # label expected-carrier
	if wait_for 60 tunnel_up && [ "$(carrier_of_a)" = "transport $2" ]; then
		...
		if [ "$pl" = "0% packet loss" ]; then
			note "L-2 $1: A is back on $2, tunnel $pl"
		else
			miss "L-2 $1: back on $2 but tunnel loss: $pl"
		fi
	else
...
for n in a b; do
	if [ "$(docker inspect -f '{{.State.Running}}' "$PFX-$n" 2>/dev/null)" = true ]; then
		continue
	fi
	miss "L-1: node $n died on B's reload under an https link"
done
...
if docker logs "$PFX-a" 2>&1 | grep -q "https: authenticated\|transport https"; then
	miss "M5-7: A established https through the bump?!"
else
	note "M5-7: https through the bump failed (no 101), as it must"
fi
...
setvpn a "$A_VPN"
sleep 1
if docker exec "$PFX-a" ping -c2 -W2 "$B_VPN" 2>&1 | grep -q "0% packet loss"; then
	note "M5-7: tunnel up over https after the pin"
else
	miss "M5-7: tunnel not up after the legitimate dial"
fi
...
if [ "$udp" = 0 ]; then
	...
	miss "UDP seen on the port ($udp)"
fi
if [ "$idleak" = 0 ]; then
	note "no cleartext tinc ID line on the wire"
else
	miss "cleartext tinc ID line leaked"
fi
if [ "$nameleak" = 0 ]; then
	note "no cleartext key material on the wire"
else
	miss "cleartext key material leaked"
fi
...
pl=$(docker exec "$PFX-a" ping -c5 -i0.2 -W2 "$B_VPN" 2>&1 | grep -o '[0-9]*% packet loss')
if [ "$pl" = "0% packet loss" ]; then
	note "L-2 residual: tunnel $pl"
else
	miss "L-2 residual: tunnel loss: $pl"
fi

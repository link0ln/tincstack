...
- [x] 🟠 Surface the new transport options in the config editor —
  **editor half**. `backend/transports.py` (Qt-free schema: `Transports`
  ...
  `[plain]`, `Obfs*`, `HttpsFront*` (*a key no core reads -- see Known
  Issues, fixed 2026-09-23: now `HttpsPort`*), `TlsCert`/`TlsKey`, `HttpsDecoy*`,
...
- **Found during M8:**
  - ~~🟠~~ *(fixed 2026-09-24: OpenSSL 3.5 replaces LibreSSL)* (build, stream Y, 2026-09-16) **the documented Android build is
  ...
  - ~~🟠~~ *(fixed 2026-09-24 in the core, see Known Issues)* (core, stream Y, 2026-09-16) **the core cannot write keys inside an
...
- 🟠 **NAT traversal, measured end to end (stream N, 2026-09-26).** Full trace
  ...
  `masqfw`, `cgnat`; `--transport`, `--ipv6`, `--capture`, mesh, rekey,
  punch), `testing/nat-sim/results/2026-09-26/summary.md`. Matrix unchanged
  since 09-16 (core 17/25 direct, median 6 s; upstream 17/25, median 12 s);
  laptop and glare: core PASS, upstream FAIL / 90 s. Everything is Linux 6.8
  netfilter only -- no real CGN, router or other kernel measured.
  - [x] 🔴 **https (and `TCPOnly`) through a relay delivered nothing.** A relay
    forwarded a TCP-borne SPTPS packet only with a key of its own to the
    destination, which it never makes with a TCP-only neighbour; inherited
    from upstream. Fixed (e5bc041, PATCHES §32). Proof: `matrix --transport
    https` 4 pairs ping 0 -> 4/4 PASS; `--node-conf TCPOnly=yes` ping 0
    (core and upstream) -> ok; re-verified on the merged tree (`nat-https`,
    `nat-quick`, laptop, glare, smoke all exit 0).
  - [ ] 🟠 **The direct peer-to-peer path is cleartext tinc UDP under every
    carrier.** `--capture` + `dpi-fingerprint`: datagrams opening with 6 zero
    bytes plain 111/181, obfs 56/417, quic 97/304; 51-byte probes everywhere;
    a constant source id under quic. Breaks the owner rule for every node
    that gets a direct path -- and every traversal improvement below adds
    more of it. **Owner 2026-09-26: seal direct datagrams as obfs does
    (frame v3)** -- stream N2. Code: `send_sptps_data`, `try_udp`,
    `obfs_wrap_send`.
  - [ ] 🟠 **quic hands its QUIC flow out as the node's UDP address.** The
    relay records the address a quic DATAGRAM came from as the node's UDP
    address and gives it out as a hint; `restricted x restricted` and
    `fullcone x fullcone` fall to relay under quic. Proof to reach: `matrix
    --transport quic --pairs "restricted/restricted fullcone/fullcone
    masqfw/masqfw"` all direct. Code: `transport_quic.c cb_recv_datagram`,
    `handle_incoming_vpn_packet_decap`, `send_udp_info`.
  - [ ] 🟠 **Relay-only `masq x masq`, `masq x portrestricted`, `cgnat x
    cgnat` fail on packet order, not on mapping.** tinc probes before the
    peer's first packet arrives and repeats every 2 s, and the early
    unsolicited probe poisons the peer's NAT. A tinc-less harness: 0/3, 2/3,
    0/3 uncoordinated, 3/3, 3/3, 2/3 with a synchronised start (rtt 0); real
    tinc at rtt 40: core 0/3, 2/3, 2/3 (n=3, not significant vs upstream).
    Fix: the relay signals "go" to both sides, no unsolicited probes, back-off
    > 30 s after failure. Code: `try_udp`, `choose_udp_address`,
    `udp_info_h`.
  - [ ] 🟡 **A rekey resets a confirmed direct path** (`KeyExpire 20`,
    `UdpMetaFallback=no`: 8.6 % of packets via relay; upstream 30.7 %).
    `ans_key_h` should apply an address only while `!udp_confirmed`.
  - [ ] 🟡 **One-sided dual-stack is worse than no IPv6** (`masq x
    restricted` goes relay, 298 "Network unreachable"); probe both families.
    Both sides dual-stack: `masq x masq` and `sym x sym` direct in 4 s.
  - [ ] 🟡 `make check` should run `masqfw/masqfw` and the quic/https rows.
  - [ ] 🟢 UPnP/PCP (needs an IGD in the lab first); one-sided punch before
    a key (measure first). Port spraying for symmetric NATs: **not to be
    done** -- lab-only gain (Linux keeps 655 in 600-1023), a port scanner on
    a real wire.
  - Risk (🟠): without `sf` links a relay outage also cuts **direct** pairs
    for ~31 s (reachability follows the meta graph): mesh `--relay-down 20`,
    upstream and core with `UdpMetaFallback=no` 0 replies for 30.5-30.8 s;
    core default 0 loss.

- 🟠 **quic, after stream Q (2026-09-26), open:**
  - [ ] 🟠 the dialler's ACKs of tunnelled TCP are 130-B datagrams in 52 % of
    its packets where curl's are 48-B ACKs -- a design question (ACK
    thinning or aggregation), not cheap.
  - [ ] 🟡 the listener answers ~RTT/8 after nginx on every h3 row (+26 ms at
    RTT 200, AUC 1.00); cause unmeasured.
  - [ ] 🟡 the authenticator travels twice (cookie + body, ~+66 B) for
    older listeners; drop the body copy once no listener from before
    2026-09-26 is left.
  - [ ] 🟡 meta PING/PONG every `PingInterval`: the server writes on the
    request stream unasked.
  - [ ] 🟢 an idle quic link sends every 15 s; a NAT with a UDP timeout
    below that loses the binding -- untested (N2 lab).
...
- 🟢 **`quic-listener-wire-test.sh` "first 1-RTT datagrams" failed once on
  ...
  then the answer; re-run 3/3 PASS with nginx's 596 + 31. 1 in 4 runs; a
  prober that times it may see the same on the wire -- unmeasured whether
  nginx ever coalesces. Measure over ~20 runs before deciding it is test
  noise.

- 🔴 **quic: stream bytes moved under ngtcp2 -- use-after-free, dead links
  under loss** (found 2026-09-26 by wave W stream A, **fixed** the same day by
  stream Q, commit c831d3a). ngtcp2 retransmits stream data from the pointers
  it was given until they are acknowledged; the meta stream's TX ring
  memmove()d unacknowledged bytes on every ack and xrealloc()ed on growth,
  and the listener's QPACK decoder stream (added 2026-09-25, 7ff64c2) --
  which ANY unauthenticated HTTP/3 client makes grow through the dynamic
  table -- xrealloc()ed too. Measured: TCPOnly, 10 % loss + 10 ms: quic
  2/2000 pings and a dead link that still counts as activated (https
  2000/2000); an aioquic client using the dynamic table under loss against an
  ASan tincd: `heap-use-after-free` read in `ngtcp2_pkt_encode_stream_frame`
  <- `quic_flush`, freed by `xrealloc` in `decoder_append` -- freed heap bytes
  could go into packets to an unauthenticated peer. Blast radius: every quic
  link on a lossy path; every quic listener to any prober since 2026-09-25
  (7ff64c2 was pushed with it). Fix: `quic_txq.c`, a chunk list whose chunks
  never move and are freed by the acknowledged offset, for the meta,
  unidirectional and decoder streams. Proof: `testing/transports/quic-loss-test.sh`
  (new): master 5076472 exit 1 in both sections, the fix exit 0 (2000/2000,
  dialled once; ASan clean);
  `testing/fingerprint/results/2026-09-26-quic-q/item0-loss/`.
- 🔴 **obfs looked like obfs, not like random UDP** (found 2026-09-26 by
  stream F, **fixed** the same day by stream O, obfs frame v3): `clen` in
  clear (`bytes[8:10] == len-10` in 9283/9283 datagrams), the whitened
  counter left bytes 0-5 constant per flow (4620/4643 consecutive pairs), and
  sizes were inner sizes + a constant. Frame v3: nonce+clen masked with a
  ChaCha20 block over a 16-byte ciphertext sample under a per-direction
  header key (QUIC's header protection), random per-datagram tail, handshake
  frames padded; receivers read v2 and v3, no version field on the wire.
  After: 0/9313, 0/9324 (shaped), 0/4657 equal prefixes, 4500 distinct
  2-byte prefixes (random ~4495), 130 bulk sizes (was 2), PMTU unchanged
  1273/1413; fuzz_obfs 10M + 4.6M runs (a build without the mask fails its
  self-test). Price: +32 B average per datagram, +128 per handshake frame; a
  current dialler cannot use obfs towards a not-yet-upgraded acceptor that
  never reached it (one fallback per reconnect) -- upgrade founders and
  relays first. `ObfsTransport/InitHeaderJunkSize` is now the tail's maximum.
  Proof: `testing/fingerprint/results/2026-09-26-obfs-v3/README.md`,
  `mixed-version-test.sh` obfs pairs, obfs-test/-mtu/-restart/-rekey,
  classify, matrix. Open (🟡): full-size datagrams get no tail while the
  junk setting is 0 (owner's call: MTU cost); bulk sizes still two 65-B
  bands; the idle heartbeat cadence (`net_packet.c`).

- 🟢 **`https-carrier-test.sh` "L-2 residual" can MISS its precondition**
  (found 2026-09-25 in the transport-parameters regression). The case needs
  B's `plain` autoconnect to reach A before A's own https re-dial after
  B's `kill -9`; once in that run A's re-dial won (`Carrier candidates for
  nodeb: https,plain ... last activated https`), so the rule under test never
  fired and the case reported MISS -> FAIL, although the outcome it guards
  was right (both ends https, 60 s stable, 0% loss). Passed on 2 re-runs.
  Impact: a false failure 1 time in ~N regressions (N unmeasured; first seen
  after ~15 green runs of this test); no product defect. Fix when it recurs:
  hold A's re-dial (e.g. block A's outbound 443 for the first seconds)
  until B's plain link is in.
...
- 🟡 **Code review of 2026-09-21 — the findings not yet acted on.** Whole-repo
  ...
  - 🟠 **Windows writes the config with inherited ACLs.** `yamlconf.c`
    ...
    **Fixed 2026-09-26 (stream C)** in C (`yamlconf.c write_atomic`:
    CreateFile CREATE_NEW with `private_sd()`, a protected DACL: SYSTEM +
    Administrators, + the user only when unelevated, owned by Administrators
    when elevated; never an inherited ACL) and in the GUI's Python writers. Wine
    proof: the old `tinc.exe` wrote `...(A;;0x1200a9;;;WD)` (Everyone read), the
    new one no WD/BU/AU ACE (`windows-wine-test.sh` 16/16). Not observable in
    Wine: the P flag, the BA owner and the elevated branch (TokenElevation
    fails there) -- check with icacls on real Windows.
  - 🟡 `json_parse(text, len)` can read past `end`: `strtod()` is called on
    ...
    **Fixed 2026-09-26 (stream C):** RFC 8259 number grammar scanned within `end`, then `strtod` on a copy; `test_json` 8 cases (5 fail on the old json.c).
  - 🟡 `json_parse` accepts trailing garbage after the top-level value (no
    `j.p == j.end` check).
    **Fixed 2026-09-26 (stream C):** `j.p != j.end` rejects it (`test_json`).
  - 🟡 `httpc` silently accepts a body shorter than `Content-Length`, turning a
    truncated response into "the server sent bad JSON".
    **Fixed 2026-09-26 (stream C):** `httpc_parse_response()` rejects a short body, an unterminated chunked body and a malformed Content-Length (HEAD/1xx/204/304 excepted); `test_httpc` 10 cases.
  - 🟢 `routes.apply_now()` passes `via` to `nexthop=` without checking it is an
    ...
    **Fixed 2026-09-26 (stream C):** the next hop must be an IP of the same family (`test_routes`).
  - 🟢 `routes.apply_now()`'s fallback (`netsh ... set route`) drops the nexthop.
    **Fixed 2026-09-26 (stream C):** the `set` fallback carries the next hop.
  - 🟢 `runtime.py:284` discards the success flag of the MTU clamp; a failed
    clamp is logged like a successful one.
    **Fixed 2026-09-26 (stream C):** a failed clamp is logged as FAILED.
  - 🟢 `certcmd.c` shadows three tinc globals (`force`, `myname`, `line`).
    **Fixed 2026-09-26 (stream C):** two real shadows renamed (`-Wshadow` 3 -> 0 warnings); `line` was a false positive (the global is meant).
  - 🟢 Dead code, the whole sweep's yield: `json_type()` (used nowhere),
    ...
    `sendmmsg` patch.
    **Fixed 2026-09-26 (stream C):** `json_type()` and `typing.Any` removed.
...
- 🟠 **Standing requirement (owner, 2026-09-23): every carrier looks on the
  ...
  - [x] 🟠 the decoy answers like no real server (item below) -- **fixed
    2026-09-23**, see "The decoy (active), confirmed" below
  - [x] **Measured 2026-09-23** — `testing/fingerprint/run.sh`, proof in
    ...
    - [x] 🔴 **Port.** A founding node listens on 655, tinc's IANA port
      ...
      **Fixed 2026-09-23** (docs/transports.md §3.1, PATCHES.md §14): a
      ...
      Limits, open:
      - [x] 🟡 the advertisement is the bound port: an operator port-forward
        ...
        not covered)
        **Fixed 2026-09-26 (wave W, stream H):** `HttpsPortPublic` /
        `QuicPortPublic` (server options, per node, not propagated; compose
        `FRONT_PORT_PUBLIC`) are advertised instead of the bound port for a
        listening front; a front that cannot bind stays unadvertised. Proof:
        `front-port-test.sh` 24/24 (listens on 443, nothing on 8443, advertises
        8443 across a restart, the invitation carries 8443 but not the options,
        https and quic dials go to 8443 only and get in through a DNAT to 443).
      - [x] 🟡 the Docker invitee also reserves host 443 (compose publishes it
        unconditionally), which fails `compose up` on a host running a web
        server until `FRONT_PORT` is changed
        **Fixed 2026-09-26 (stream H):** `join.sh` adds `compose.leaf.yml`
        (`ports: !reset []`) when PORT is unset and records it in `.env`; the
        founder publishes as before. Proof: `platforms/linux/docker/leaf-ports-test.sh`
        7/7 (control: the old invitee fails "port is already allocated" on a taken
        host port). Not covered: an exported COMPOSE_FILE overrides .env (warned,
        documented).
      - [ ] 🟡 `tinc join` is still cleartext tinc on the tinc port (item below)
      - [x] 🟠 non-TLS bytes on TCP 443 are closed without a word; nginx
        answers `400` (decoy step, next) -- **fixed 2026-09-23**: plain
        HTTP and a tinc ID line on `HttpsPort` get nginx's two `400` pages
        byte for byte, a `0x80`+ first byte is closed silently as nginx does
        (`decoy-conformance-test.sh` "plain"/"tincid"/"non-ASCII" cases;
        `front-port-test.sh` now expects the `400` instead of silence)
      - [ ] 🟡 Windows: `SO_REUSEADDR` on the TCP listeners lets another
        ...
        **Code fixed 2026-09-26 (stream H), stays open:** `SO_EXCLUSIVEADDRUSE`
        on every Windows listener, TCP and UDP (`net_socket.c set_bind_policy()`).
        Under Wine the old `tincd.exe`'s UDP 655 is taken by a second process with
        SO_REUSEADDR, the new one refuses all four ports (`windows-wine-test.sh`
        19/19; the same test on `core-win:dev` fails exactly that check). Open: Wine
        refuses a second TCP listener like Linux, so the TCP half needs real
        Windows -- and so does the cost Microsoft documents for this option: a port
        may stay unbindable after close while accepted connections linger, and the
        GUI restarts tincd on every config change (🟡, measure before a release).
    - [ ] 🟠 **Our QUIC ClientHello claims `h3` and breaks HTTP/3.** Transport
      ...
      - [x] 🟡 Measured again 2026-09-24 (`android-emulator-test.sh`): curl
        ...
        active_connection_id_limit 2, no datagrams); ours, ngtcp2's,
        `15,5,6,7,4,8,9,1,14,32,17` -- 10 bytes longer, with
        `max_datagram_frame_size` (the data path needs it) and
        `version_information`. JA4 and the Initial's size match, the
        decrypted Initial does not. Matching it means leaving ngtcp2 or
        ...
        **Fixed 2026-09-24 (owner: "patch ngtcp2 for curl's transport
        ...
        - [x] 🟡 **The second flight.** The client Initial coalesced with
          ...
          with no decryption at all.
          **Fixed 2026-09-24, late (owner: "fix the second flight's padding
          in ngtcp2").** Measuring it found two more differences in the same
          datagram, both fixed too: OpenSSL moves to the connection ID the
          server issues in its first 1-RTT packet (NEW_CONNECTION_ID seq 1)
          at once, so that datagram's Destination Connection ID -- in the
          clear -- is new (ngtcp2 kept the original); and ngtcp2 put a PING
          into an ACK-only Initial when the server's flight took longer than
          its RTT estimate (seen 2 of 5 lab runs), which OpenSSL does not.
          The patch (openssl-client mode only) writes the Handshake packet
          and an ACK-only 1-RTT packet into a scratch buffer, then the
          Initial padded to what is left; never adds that PING to an Initial
          or Handshake packet; moves to the first issued connection ID
          without retiring seq 0. Proof: `quic-wire-test.sh` now requires
          the datagram to equal curl's -- 1208 B, Initial+Handshake+1-RTT,
          Lengths 1051 and 80, the Initial ACK + PADDING 1026, the DCID
          moved -- 10/10 runs plus the regression's; the previous core fails
          it (`25,80`, `ACK`, `kept`). tshark cannot follow a connection
          across the DCID move without the TLS secrets (it had silently shown
          curl's Initial frames as empty), so the Initials are decrypted by
          the new `testing/transports/quic_initial.py` (stdlib, RFC 9001
          vectors checked). `windows-wine-test.sh`: the Windows datagram
          equals the Linux one. ngtcp2's own suite on the patched tree:
          green. Regressions on this core (`regression-summary.txt` in
          `testing/fingerprint/results/2026-09-24-second-flight/`): 32 test
          runs, 31 exit 0 first time, incl. `mixed-version` against `pre-sf2` (the core before
          this change), `pre-tp`, `pre-deb13`; `app-quic` failed once in UI
          automation before any traffic and passed on re-run.
          `tincmgr.exe` rebuilt with this core (not in git): 62 023 971 B,
          SHA-256 `b814d6f6...`, `tincd.exe` `385d6abb...`; Wine selftest OK.
        - [ ] 🟡 **The second flight on a slow client (Android emulator).**
          6 of 7 emulator runs sent no Initial with the Finished: the
          phone's third Initial ACKs server Initials 0-1 only, then a 50-byte
          Handshake-only datagram, then the Finished as Handshake + 1-RTT in
          146 unpadded bytes; the server's Initial 2 is never acknowledged.
          The emulator needs ~25 ms per server datagram; by all signs
          (packets only, no ngtcp2 log taken) the PTO fires first, the
          client probes in Handshake and drops its Initial keys. Whether
          OpenSSL does the same in that timing is unmeasured (no curl on the
          phone; reproducing it needs a slowed Linux client next to a slowed
          curl). A real phone on a real network (RTT >> processing time)
          probably takes the normal path -- not measured.
          `android-emulator-test.sh` prints it as NOT COMPARED, not PASS.
          **Reproduced 2026-09-26 (stream F), not fixed:** under `--cpus 0.03` our dialler sends a Handshake-only datagram in
          8/26 dials, curl 0/26 (p ~ 0.002); at 1 CPU 0/26 each. It is ngtcp2's PTO
          probe in openssl-client mode; fix in `tincstack-wire.patch` (moderate).
        - [ ] 🟡 **The first 1-RTT packets** (HTTP/3 control, QPACK, the
          request): 54, 41, 41, 315 UDP bytes from us, 55, 40, 40, 69 from
          curl (re-measured 2026-09-24, late, after the second flight) --
          ...
          key log).
          **Re-measured 2026-09-26 (stream Q):** 66, 53, 53, 397 from us, 67,
          52, 52, 81 from curl. The request grew: the authenticator now rides
          both a cookie and the body (the body copy is for older listeners),
          and it is a POST with a browser UA where curl GETs -- matching curl's
          69-81 B is not possible for a request that carries a token. The +-1 B
          on the first three is not decoded. Open.
        - [ ] 🟠 **The listener's QUIC side is not nginx's** (was 🟡
          "never compared"; measured 2026-09-25 with the new
          `testing/transports/quic-listener-wire-test.sh`: our founder next
          to Debian 13's own nginx 1.26.3 on OpenSSL 3.5.7 with the
          founder's certificate and key, curl with a key log, raw probes from
          `quic_probe.py`; 0 of 9 checks pass, the nginx side identical in 3
          runs; `testing/fingerprint/results/2026-09-25-quic-listener/`).
          2 of 9 pass since the probe and decoy fixes below
          (`.../2026-09-25-quic-listener-fixed/`), 4 of 9 since the
          handshake fix (`.../2026-09-25-quic-handshake/`), 6 of 9 since
          the transport parameters and SETTINGS (`.../2026-09-25-quic-tps/`),
          9 of 9 since stream A (2026-09-26, `.../2026-09-26-quic-tickets/`).
          The POST-405 timing and the decoy's HTTP/3 answer framing, open
          after stream A, closed by stream Q (2026-09-26, 10/10 on its image;
          on the merged tree the first-1-RTT check failed once on ACK timing and
          passed 3/3 on re-run, see Known Issues).
          Official `nginx:1.29` (1.29.8, Debian 13, OpenSSL 3.5.5) behaves
          the same on every QUIC point below; `nginx:1.27` (OpenSSL 3.0) does
          not pick X25519MLKEM768 and is the wrong reference for us.
          - [x] 🟠 **Silent to one-datagram probes.** nginx answers an
            unknown version (0x1a2a3a4a, draft-29, v2; even in a 1199-byte
            datagram) with Version Negotiation listing 00000001 and echoing
            both ids, and a short-header datagram for a connection it does
            not have (1200, 100, 40 bytes, even the 27-byte text `GET /
            HTTP/1.1`, whose first byte looks like a short header) with a
            stateless reset. Our listener answers none of them. One UDP
            packet, no handshake, tells us apart.
            **Fixed 2026-09-25:** the QuicPort socket sends Version
            Negotiation and stateless resets to nginx's measured thresholds
            (docs/transports.md §9.8). Proof: `quic-listener-wire-test.sh`
            "answers to one-datagram probes" PASS, all 12 probes line for
            line nginx's; the threshold sweep (VN at 7/20/23..1500 B, ids
            0/1/20/21; resets at 20..1500 B x5, a burst of 30, v1 Handshake
            junk; `testing/transports/quic_thresholds.py`) gives the same
            lengths and silences on both servers
            (`testing/fingerprint/results/2026-09-25-quic-listener-fixed/`).
            Tinc's own port still answers nothing (its unclaimed datagrams
            are SPTPS's and obfs's) -- that port is not a web port.
          - [x] 🟠 **The HTTP/3 decoy answers 200 to everything.** `GET
            /nope`, `POST /`, `DELETE /` get the 615-byte page with 200;
            nginx: 404 (146 B), 405 (150 B), 405. `HEAD /` gets a body, a
            protocol violation curl reports as `ERR_MALFORMED_HTTP_MESSAGING`
            (nginx: 200, no body). The TCP decoy answers all of these as
            nginx does (`decoy-conformance-test.sh`); the HTTP/3 one does not
            use it. Care: the tinc session is itself a `POST /`, so a POST
            whose authenticator fails must end in nginx's 405, at nginx's
            timing.
            **Fixed 2026-09-25, except the timing:** the request's QPACK
            field section is decoded (`h3_decode_request`, static table,
            literals, Huffman) and its method, path and authority go to the
            TCP decoy's responder. Proof: `quic-listener-wire-test.sh`
            "HTTP/3 answers" PASS (GET / 200 615 B, GET /nope 404 146 B,
            HEAD / 200 no body, POST / and DELETE / 405 150 B, header names
            nginx's); `test_h3.c` 4/4 (RFC 7541 vectors, malformed
            sections, the dialler's own request).
          - [x] 🟡 **A POST's 405 comes after its body, nginx's right after
            its HEADERS.** A POST is the tinc session's method, so the
            listener must see the authenticator before answering; nginx
            answers a POST to a static file without reading the body. A
            prober timing HEADERS-then-pause-then-DATA sees the difference.
            Unmeasured how large it is on a real path. Answering early would
            need the dialler to be told apart by HEADERS alone (a path or
            header only a dialler sends), which is itself a tell --
            owner's call.
            **Measured 2026-09-26 (stream F), raised to 🟠:** a head-only POST is
            never answered by us (54/54 probes, 2/2 waited 70 s), nginx answers 405 in
            one RTT; a body 1 s after the head: ours 1018-1203 ms, nginx 21-204 ms,
            12/12 in every RTT x jitter cell -- one probe settles it. The TCP front is
            indistinguishable (378/378). **Owner 2026-09-26: carry the authenticator
            (an exporter-bound token) in the request HEADERS** -- stream Q item 1.
            **Fixed 2026-09-26 (stream Q, 1816c52):** the dialler's authenticator
            rides a `sid=` cookie in HEADERS and the listener decides there. At
            RTT 200: post-pause 1201.5 -> 228.1 ms (nginx 200.8), a head-only POST
            now gets 405 in 227.6 ms (nginx 202.6); the structural tell is gone in
            every cell. Residual: the listener still answers ~RTT/8 after nginx on
            every h3 row (🟡, below). Proof:
            `testing/fingerprint/results/2026-09-26-quic-q/post405-*`.
          - [x] 🟢 **If SETTINGS ever announce a QPACK dynamic table** (nginx
            announces 4096/128, see below), `h3_decode_request` must learn
            the encoder stream and dynamic references; today such a
            reference is invalid and the request is answered 400 at FIN.
            **Done 2026-09-25**, with the SETTINGS below: `h3.c` keeps the
            client's dynamic table (the four encoder-stream instructions,
            capacity up to 4096, blocked sections up to 128 streams, post-base
            references) and writes nginx's decoder-stream instructions
            (Section Acknowledgment, Insert Count Increment, Stream
            Cancellation); a capacity above ours closes with
            ENCODER_STREAM_ERROR, a 129th blocked stream with
            DECOMPRESSION_FAILED, as nginx's source does. Proof: `test_h3.c`
            `test_dynamic_table` (RFC 9204 Appendix B's capacity 220
            `3f bd 01`, inserts, a blocked section released by a split
            instruction, Section Ack `84`, post-base, Duplicate + Insert
            Count Increment `01`, the errors, Stream Cancellation `48`) under
            ASan/UBSan; Chromium, which uses the table, still gets the page
            (`h3-interop-test.sh`).
          - [x] 🟠 **The handshake, in the clear.** Our connection ids are 8
            bytes, nginx's 20 (every long header). Our long-header Length
            fields are 4 bytes, nginx's minimal (the ACK-only Initial: 50 vs
            59 B UDP). Our third datagram is Initial + Handshake (the whole
            flight in one CRYPTO frame, unpadded) + a 1-RTT packet
            (NEW_CONNECTION_ID in 0.5-RTT, then PADDING); nginx's is Initial
            + Handshake padded to fill the datagram, one CRYPTO frame per TLS
            message (EE, Certificate, CertificateVerify, Finished), no 1-RTT.
            Packet types and Lengths need no key.
            **Fixed 2026-09-25:** `TRANSPORT_QUIC_CIDLEN` 20, and a new
            server mode in `core/ngtcp2/tincstack-wire.patch`
            (`ngtcp2_conn_set_nginx_server_wire`: minimal Length, a CRYPTO
            frame per TLS message, nothing in 1-RTT before the handshake
            completes, so the Handshake packet takes the padding). Proof:
            `quic-listener-wire-test.sh` "connection id length" and
            "handshake flight" PASS; decrypted with curl's key log, both
            servers send 59 / 1208 (Initial 1170) / 1208 (Initial 55 +
            Handshake 1087: CRYPTO 34 | 125, 409, 78-79 (an ECDSA signature), 52,
            PADDING), nginx's
            EE 121 B -- the transport parameters, encrypted, own item below
            (`testing/fingerprint/results/2026-09-25-quic-handshake/`).
            Price: tinc's PMTU over quic 1119 (was 1131; `quic-wire-test.sh`),
            every dialler packet carrying 12 more bytes of id. The dialler
            against the new listener is still curl's (`quic-wire-test.sh`
            PASS), and old nodes still connect both ways (mixed-version vs
            `pre-tp`, `pre-sf2`, `pre-deb13` and the previous core).
          - [x] 🟡 **What a prober reads after a handshake.** (Split
            2026-09-25: transport parameters and SETTINGS fixed, sub-items
            below; tickets, first 1-RTT datagrams and the wrong-ALPN close
            open.) Transport
            parameters: ours `original_destination_connection_id`,
            `stateless_reset_token`, `initial_source_connection_id`, stream
            windows 524288, `initial_max_data 786432`, 100/3 streams,
            `max_idle_timeout 30000`, `active_connection_id_limit 8`,
            `max_datagram_frame_size 65536`, `version_information`; nginx
            `initial_max_data 8585216`, 3/128 streams, windows 65536,
            `max_idle_timeout 75000` (65000 on 1.29), `max_udp_payload_size
            65527`, `active_connection_id_limit 2`, `max_ack_delay 25`,
            `ack_delay_exponent`, then the ids and the token. HTTP/3
            SETTINGS: ours QPACK 0/0, `MAX_FIELD_SECTION_SIZE 65536`,
            `H3_DATAGRAM 1` (nginx never sends it); nginx QPACK 4096/128.
            Session tickets: 7200 s, 208 B vs 300 s, 224 B. First 1-RTT
            datagrams: ours 514 (HANDSHAKE_DONE, CRYPTO, STREAM) + 38 + 38 B,
            nginx one 596-byte packet (2 CRYPTO tickets, HANDSHAKE_DONE,
            NEW_CONNECTION_ID, 3 STREAMs with offsets). A wrong ALPN: both
            close with 0x178 in an Initial, nginx with the reason `handshake
            failed` (75 vs 50 B). Cost to fix the datagram points: the
            listener would have to stop announcing `max_datagram_frame_size`
            and `H3_DATAGRAM`, which diallers from before 2026-09-24 rely on
            -- a compatibility cut to plan, not a side effect.
            - [x] 🟡 **Transport parameters.** **Fixed 2026-09-25**
              (`ngtcp2_transport_params_encode_nginx_server` in
              `core/ngtcp2/tincstack-wire.patch`, values in `quic_settings`):
              nginx 1.26.3's set, order and values, every integer written,
              no `max_datagram_frame_size`, no `version_information`; the idle
              timeout stays above nginx's 75 s only if 3 x PingTimeout does.
              Proof: `quic-listener-wire-test.sh` "the server's transport
              parameters is nginx's" PASS; EE 121 B on both, decrypted with
              curl's key log (`testing/fingerprint/results/2026-09-25-quic-tps/`).
            - [x] 🟡 **HTTP/3 SETTINGS.** **Fixed 2026-09-25:** the listener's
              control stream carries nginx's `04 06 01 5000 07 4080` (QPACK
              4096/128, nothing else), and it opens a QPACK decoder stream and
              no encoder stream, as nginx; the table is real (item above).
              The dialler's SETTINGS are unchanged. Proof:
              `quic-listener-wire-test.sh` "the HTTP/3 SETTINGS is nginx's"
              PASS; Chromium, curl and nginx interoperate (`h3-interop-test.sh`).
            - [x] 🟠 **The compatibility cut, planned.** Without
              `max_datagram_frame_size` in the listener's parameters a dialler
              from before cannot send DATAGRAM frames. Measured on the first
              build of this change: a `pre-tps` dialler connects over quic,
              its link dies on the first tunnel packet, and it dials quic
              again for ever (7 dials in 80 s, the tunnel losing packets) --
              and `mixed-version-test.sh` still said PASS, because it only
              looked for "transport quic" once. Fixed: a current dialler
              sends `H3_FRAME_TINC_DGRAM` before its authenticator; a
              listener that authenticates a dialler without it answers as a
              web server (405), and the old dialler, which gives up on any
              web server's answer, moves to its next carrier. The test now
              pings through every carrier and counts dials. Proof:
              `mixed-version-test.sh` against `pre-tps`: "new founder, old
              leaf" over quic falls back to plain once, tunnel carries
              traffic; the same test on the first build: FAIL (ping ok, then
              no connection). Price: nodes from before 2026-09-25 lose quic
              towards upgraded listeners (their other carriers work) --
              upgrade diallers too.
            - [x] 🟡 **Session tickets.** **Fixed 2026-09-26 (stream A)**
              (`transport_quic_tls.c` `nginx_session_cache`: nginx 1.26's
              ssl_session_timeout 5m, SHA-1 session id context, the cache mode of
              `ssl_session_cache none`): 300 s, 224 B, two tickets, as nginx (were
              7200 s, 208 B). Proof: `quic-listener-wire-test.sh` "the session tickets"
              PASS (`testing/fingerprint/results/2026-09-26-quic-tickets/`).
            - [x] 🟡 **First 1-RTT datagrams.** **Fixed 2026-09-26 (stream A)**
              (`ngtcp2_conn_set_nginx_server_wire`: CRYPTO first, one per ticket,
              HANDSHAKE_DONE before NEW_CONNECTION_ID, STREAM frames always with
              Offset, padding only for header protection; `transport_quic.c` packs the
              uni streams into one packet): 596 B `06,06,1e,18,0e,0e,0e` then 31 B
              `02`, as nginx (were 549 B `18,1e,06,0a` + 50 B). Proof: same test PASS.
            - [x] 🟡 **Wrong-ALPN close.** **Fixed 2026-09-26 (stream A):** TLS-alert
              closes carry nginx's reason `handshake failed`: 75 B, 0x178, Initial.
              Proof: same test PASS. `quic-listener-wire-test.sh` 9/9.
            - [ ] 🟡 **The decoy's HTTP/3 answer frames.** nginx: HEADERS and DATA in two
              STREAM frames (727 B of stream for GET /), ours one (802 B); same 615-B
              page, nginx's field section 75 B shorter -- not decoded yet (stream Q
              item 5).
          - [x] 🟡 **Owner's call: which nginx we are.** Debian 13's nginx
            1.26.3 is the persona that fits our TLS stack and already serves
            our decoy's page byte for byte (only Last-Modified/ETag differ:
            2025-02-05 vs 2025-04-16), with `Server: nginx`. The decoy and
            fingerprint labs still compare against `nginx:1.27` (OpenSSL
            3.0, no post-quantum). Recommendation: move them to the Debian 13
            reference and take its page dates.
            **Decided 2026-09-26 (owner): Debian 13 nginx 1.26.3 for every carrier.**
            Done by stream E: the decoy is that nginx's static module byte for byte
            (page dates `Wed, 05 Feb 2025 11:07:30 GMT`, ETag `"67a34672-267"`, gzip
            level 1, conditional requests, ranges, directories, parse errors) and
            `decoy-conformance-test.sh` / `testing/fingerprint/run.sh` compare against
            `tincstack/nginx-deb13:dev`: 78/78 on this core, 63 of 78 fail on the
            previous one (`testing/fingerprint/results/2026-09-26-deb13-persona/`).
      - [x] 🟠 **JA4 is GnuTLS 3.7.9's and stays unique.** Not fixable
        ...
        **Fixed 2026-09-23 (owner said go; Docker images only, see below)**
        (docs/transports.md §9.1, §9.8; PATCHES.md §17): Debian 13 base,
        ngtcp2 1.25 `--with-openssl` on OpenSSL 3.5.7, GnuTLS gone from the
        image (`ldd tincd`: libssl.so.3, libngtcp2_crypto_ossl). Measured
        `testing/fingerprint/results/2026-09-23-deb13/`: our QUIC
        ClientHello JA4 `q13d0312h3_55b375c5d22e_e01b5de7605b`, JA3
        `018311bd...`, 1248 B -- identical to curl 8.14's (the first
        OpenSSL build differed by one extension, `session_ticket`; the
        dialler now sets `SSL_OP_NO_TICKET` as curl's hello shows); QUIC
        ServerHello JA3S `15af977c...` = nginx's (was GnuTLS's
        `eb1d94da...`). Before: `2026-09-23-decoy/report.txt`. Checked on
        the way: our client now tests the selected ALPN itself (GnuTLS
        enforced it); GnuTLS-era and OpenSSL nodes still connect over
        quic and https both ways (`mixed-version-test.sh`, new, 6/6 against
        a core built from 16f5f6f). Regressions on the Debian 13 core, all
        exit 0: front-port, h3-interop, https-carrier, quic-carrier,
        tls-front, plain-refuse, classify, cert-repin, carrier-switch,
        invitee-mesh, obfs, same-nat-meta, singleflow, obfs-restart,
        obfs-confirmed-peer, decoy-conformance, mixed-version, matrix
        (`core:dev-test`), acme (pebble, `httpc` on OpenSSL 3.5),
        cert-lifecycle, ping-interval, zeroconf-pool, smoke (vanilla-tinc
        baseline binary inside the trixie lab image); no gcc warnings outside
        the old `tincctl.c` ones; `make lint`, `make secrets` clean; the
        mingw build (Debian 12, gcrypt, no TLS carriers) still passes.
        Still open:
        - [ ] 🟠 **Windows and Android have no TLS carrier at all** (Windows
          core `-Dcrypto=gcrypt`, Android LibreSSL libcrypto only), so this
          fix covers the Docker images -- servers and Linux peers. The
          client side a censor sees is mostly laptops and phones: when those
          platforms get https/quic they must link the same OpenSSL 3.5
          (static, built in the build container) with the same client
          settings, or each platform gets its own ClientHello again.
          **Closed 2026-09-24 with Android** (sub-item below).
          - [x] **Windows: done 2026-09-23** (PATCHES §18, docs/transports.md
            §9.10). `core/Dockerfile.build-win` (Debian 13) cross-builds
            OpenSSL 3.5.7 + zlib 1.3.1 + zstd 1.5.7 + ngtcp2 1.25.0 from
            sha256-pinned tarballs, links them statically, `-Dcrypto=openssl
            -Dquic=enabled`; `tincd.exe --version` under Wine: `Features:
            openssl legacy_protocol`; DLLs: system ones only (+ CRYPT32).
            Proof: `testing/transports/windows-wine-test.sh` 14/14 under Wine
            (`testing/fingerprint/results/2026-09-23-windows/`): Linux leaf ->
            Windows founder and Windows dialler -> Linux founder, both over
            https and quic; curl gets the decoy from the Windows node over TLS
            and HTTP/3, plain bytes get nginx's 400; ServerHello JA3S
            `15af977c...` as on Linux; Windows ClientHellos = Linux ones =
            curl's dialling the same IP (https `t13i3012h2`, 1623 B; quic
            `q13i0311h3`, 1248 B, extension lists equal). Its first run
            failed the ClientHello check: no `compress_certificate` (Debian's
            OpenSSL has zlib+zstd, mine did not) -- hence zlib/zstd. **Not
            proven:** a real Windows kernel, and Wintun (the lab runs
            `DeviceType = dummy`). `tincmgr.exe` rebuilt with this core
            2026-09-24 (`build-exe.sh`, PyInstaller under Wine; 62 022 364 B,
            SHA-256 `817292ec...8b96e2`, not in git): Wine selftest OK, and
            ...
            1646 B, `q13d0312h3_55b375c5d22e_e01b5de7605b` 1248 B).
          - [x] **Android: done 2026-09-24** (PATCHES §19, docs/transports.md
            §9.10). `native/build-core.sh` cross-builds OpenSSL 3.5.7 + zstd +
            ngtcp2 per ABI (same tarballs and OpenSSL options as Windows),
            static; `libtincd.so` NEEDED: libc, libm, libz; `--version`:
            `Features: openssl comp_zlib legacy_protocol`. The default
            `-PtincCrypto=openssl` builds again, so CI and the release no
            longer pass `nolegacy`. Proof on the Android 14 emulator
            (`testing/fingerprint/results/2026-09-24-android/`):
            `android-emulator-test.sh` -- the NDK binaries dial a Linux
            founder over https and quic, ClientHellos = the Linux dialler's
            (JA4_r, handshake 1542 / 1487 B, QUIC transport parameters);
            `join-on-emulator.sh TRANSPORT=https|quic` -- the app joins
            ...
            **Not proven:** an ARM ABI on hardware, a mobile network; the
            ...
            22.9 MB). Regressions after the core changes (retry, tmpfile),
            all exit 0: the 24 above plus `retry-carrier`, `windows-wine`,
            `android-emulator` -- 27.
        - [ ] 🟡 not a browser: Chromium's hello (GREASE, ECH, ALPS) needs
          BoringSSL; we now look like curl/OpenSSL 3.5, a common
          non-browser client
        - [x] 🟡 the fingerprint lab's reference nginx is `nginx:1.27`
          (Debian 12, OpenSSL 3.0, no post-quantum): ours answers curl's
          `X25519MLKEM768` share with a ~660 B larger ServerHello flight,
          as a Debian 13 nginx would; the lab should add that reference
          **Done 2026-09-26 (stream E):** `testing/fingerprint/run.sh` uses the Debian 13 reference.
    - [x] 🟠 **Our QUIC server does not behave like an HTTP/3 server** (active):
      ...
      **Fixed 2026-09-23** (`h3.c`, docs/transports.md §9.4): the carrier
      ...
      new.
    - [x] 🟠 **Our TLS ClientHello is an old OpenSSL, not a browser.** JA4
      ...
      **Fixed 2026-09-23** with the OpenSSL 3.5 move (item above,
      docs/transports.md §8.2): JA4 `t13d3013h2_1d37bd780c83_8537cf56674e`,
      JA3 `32e4b881...`, 1646 B -- identical to curl 8.14's; the dialler
      offers ALPN `h2, http/1.1`, no `session_ticket`, `post_handshake_auth`,
      as curl does, and gives up on a server that selects h2 (checked
      against nginx with `http2 on`: `answered like a web server (ALPN
      h2)`). Still open:
      - [ ] 🟡 the upgrade request inside TLS (`User-Agent: Mozilla/5.0`
        alone, WebSocket to `/`) matches neither curl nor a browser; only
        a TLS terminator sees it
    - [x] 🟠 **The decoy (active), confirmed:** B answers garbage over TLS with
      ...
      **Fixed 2026-09-23** (docs/transports.md §8.5.1, PATCHES.md §16),
      except h2 (open below). The decoy is nginx 1.27 with `server_tokens
      off`: nginx's welcome page byte for byte (it was Apache's "It works!"
      under `Server: nginx`) with the stock `nginx:1.27.5` file's
      `Last-Modified`/`ETag`, nginx's header order with `Date`, `404` /
      `405` / `400` with its error pages, keep-alive with pipelining, TLS
      `close_notify` before the FIN (we sent a bare FIN), nginx's answers
      to plain bytes on the TLS port, 60 s for a silent client (was: held
      for ever by the tarpit). Found on the way and fixed: tinc's accept
      tarpit (`check_tarpit`, 10 connections/s from one address) and the
      per-second QUIC budget answered the 11th connection of a burst with
      silence -- a one-line prober check; the web fronts now skip both and
      cap concurrent unauthenticated clients at 256 instead (the tinc port
      keeps them). Proof: `testing/transports/decoy-conformance-test.sh`,
      same probes to a tinc node and to `nginx:1.27`: 20/20 on this core;
      on the core before it (`core:pre-h3`) 19/20 fail -- only the
      high-byte case matched (`testing/fingerprint/results/2026-09-23-decoy/`
      `conformance.txt`, `conformance-pre-h3.txt`; fingerprint re-run in the
      same directory: garbage over TLS now `400`, ClientHello/ServerHello
      values unchanged). Also found: the labs' `tcpdump -U` without
      `--immediate-mode` lost the tail of sparse captures (the nginx side
      showed 14 packets of ~200) -- all four capture labs use it now; and
      `h3-interop-test.sh` broke whenever the random Ed25519 key contained
      `/` (sed delimiter), fixed; and the web-front timeout first counted
      from the request, which would have cut a slow but progressing
      download at 60 s -- it now counts from the last byte written, as
      nginx's `send_timeout` does. `tls-front-test.sh` M5-8 now waits for
      that 60 s close (it expected tinc's authentication timeout).
      Regressions, all exit 0: front-port (expects nginx's `400` now),
      quic-carrier, plain-refuse, classify, cert-repin, carrier-switch,
      invitee-mesh, obfs, same-nat-meta, matrix (`core:dev-test`), smoke --
      on the core before the send-timeout change; tls-front, h3-interop,
      https-carrier and decoy-conformance (20/20) on the final core. `make
      lint`, `make secrets` clean; mingw cross-build OK. Open:
      - [ ] 🟡 no ALPN `h2`: an nginx with `http2 on` (most HTTPS sites)
        selects h2 for curl and browsers, we always answer `http/1.1`
        (matches nginx's default config, not the typical site)
        **Tried 2026-09-26 (stream G), stays open, deferred:** our dialler offers
        `h2, http/1.1` like curl and gives up when a server selects h2, so a listener
        selecting h2 would cut every current dialler off https (their ClientHello is
        curl's, the listener cannot tell them apart). h2 needs a dialler h2 path
        first (a POST stream with an exporter-bound token, as the quic carrier's
        POST token), and old diallers could only fall back. Deferred until the quic
        POST token has proven that pattern; the stream stopped before any code.
      - [x] 🟡 no `Alt-Svc` on TCP answers although the node serves HTTP/3
        on 443; sites that serve h3 advertise it (the lab's nginx does)
        **Fixed 2026-09-26 (stream E):** `Alt-Svc: h3=":<QuicPort>"; ma=86400` on the statuses the reference `add_header` adds it to, when the quic front is up (`decoy-conformance-test.sh`).
      - [x] 🟡 not imitated: `Range`, `If-Modified-Since` → `304`,
        directory redirects (`/dir` → `301`), TLS session-ticket size
        **Fixed 2026-09-26 (stream E):** 304/412, 206/416/multipart, If-Range, 301
        (absolute Location) and 403 for directories, TLS tickets 261/214 B at
        300 s as nginx (SNI callback, SHA-1 session id context,
        `SSL_CTX_set_timeout(300)`); `decoy-conformance-test.sh`.
      - [x] 🟡 the HTTP/3 decoy always serves `/` and never proxies to
        `HttpsDecoyUpstream`; with an upstream the TCP decoy relays its
        answer and always closes (the forwarded request says
        `Connection: close`)
        **Fixed 2026-09-26 (stream E):** the HTTP/3 decoy relays to the upstream
        (`h3-interop-test.sh`), and the TCP decoy with an upstream behaves like
        `proxy_pass` (HTTP/1.0 upstream request, nginx's own Server/Date, the client
        connection kept alive; `decoy-conformance-test.sh` proxy set). Open (🟡):
        over HTTP/3 only method, path and authority reach the responder, so If-*,
        Range and gzip do not apply there (stream Q item 2); a failed upstream gives
        the static page after 3 s, nginx 502/504 after 60 s.
      - [x] 🟡 the 75 s keep-alive idle close is set, not measured; the
    ...
    - [ ] 🟡 not audited yet: `obfs` (should be uniformly random: check
      ...
      **Audited 2026-09-26 (wave W, stream F; measurement only):** see
      `testing/fingerprint/results/2026-09-26-audit/README.md` for every item's
      text, repro and numbers. Findings: 🔴 obfs `clen` in clear, 🔴 obfs nonce
      ...
      1200 B in 4978/6386, nginx 3888/4599); 🟠 POST-405 timing (item "A POST's 405"); 🟡 the listener answers ~RTT/8
      later than nginx (cause unmeasured); 🟡 slow-client second flight
      reproduced on Linux (`slow-client-flight.sh`, `--cpus 0.03`: ours 8/26
      Handshake-only datagrams, curl 0/26). Stays open until those are.
...
- 🔴 **Second code review of 2026-09-22 — new findings; three fixed 2026-09-23 (renewal lock-out, QUIC pin before SPTPS, renew timer's first check), the rest open.**
  ...
  - [x] 🟠 **The elevated Windows manager runs code any unelevated process can
    ...
    **Fixed 2026-09-23** (`backend/paths.py`, `runtime.py`, `management.py`,
    ...
    What is still open:
    - [x] 🟠 **The onefile exe itself unpacks into `%TEMP%\_MEIxxxx`** and
      ...
      **Fixed 2026-09-26 (stream C):** `tincmgr.spec` builds a onedir tree and a
      onefile that carries it plus a SHA-256 manifest compiled into its PYZ;
      elevated, the onefile installs the tree into `%ProgramFiles%\tincmgr\app`
      (every file verified against the manifest, swapped in by rename) and the
      logon task runs that onedir exe, which unpacks nothing. Bundled core
      binaries are verified against the manifest too, with no fallback to an
      unverified file. Proof: `build-exe.sh` SMOKE under Wine 14/14 (installed
      copy runs from Program Files, meipass `app\_internal`, no `_MEI*` in
      %TEMP% before/after), `test_trust`. Not verified: that real Windows denies
      an unelevated write to `Program Files\tincmgr\app`, and that a running copy
      blocks the swap. Residual (🟠, inherent to onefile): the FIRST elevated run
      of a downloaded onefile still loads its DLLs from the user's %TEMP% -- only
      a signed installer/MSI closes that; the permanent logon-task window is
      closed. Residual (🟡): when staging fails because a file is busy, the
      hash-verified `_MEI` tincd runs, with a TOCTOU window between check and
      launch.
    - [x] 🟢 Last run wins: running an older tincmgr elevated with autostart
      on puts that older copy into Program Files.
      **Fixed 2026-09-26 (stream C):** the automatic refresh never installs an older or a dev build over a newer release (`test_refresh_never_downgrades_but_the_toggle_may`); toggling autostart off and on is the explicit way to downgrade.
    - [x] 🟢 `refresh_startup_task()` runs `schtasks` on the Qt thread at start
      (one `/query`, plus `/create` and PowerShell once when migrating).
      **Fixed 2026-09-26 (stream C):** refresh, toggle and state query run in the WorkerPool (`test_startup_task_refresh_runs_off_the_qt_thread`, `test_startup_state_query_runs_off_the_qt_thread`).
  ...
  - [x] 🟠 **`tinc retry` during a TLS or QUIC handshake demoted the link to
    plain for the session.** *(Found and fixed 2026-09-24, every platform.)*
    `retry()` expires connections still being set up; the carrier selector
    counted that as a failed handshake and fell back, and an activated plain
    link is never re-dialled. The Android app runs `tinc retry` on every
    connectivity change, the VPN coming up included: its https link ended on
    plain on every start (emulator, app log: `Got CONTROL ... 18 10` ->
    `Timeout ... during authentication` -> `Carrier https failed`). Now
    `retry()` marks them and the re-dial keeps the carrier.
    `testing/transports/retry-carrier-test.sh` (400 ms netem, retry mid-
    handshake): the previous core ends on plain for https and quic, this one
    on https and quic.
  - [x] 🟡 **The Android app made the network's config directory world-
    readable and -writable** (`tinc.yaml` 0666, directories 0777, private
    keys inside). *(Found and fixed 2026-09-24.)* The join and
  ...
  - [x] 🟠 **The https dial socket stayed blocking on Windows.** *(Found and
    ...
    whole event loop. Measured under Wine: with the founder dropping TCP 443,
    the Windows node's own front took 10 s to time out a curl on the build
    without the fix, 0.45 s with `ioctlsocket(FIONBIO)`
    (`windows-wine-test.sh`, last check). Had no effect before: Windows had
    no https carrier.
  - [x] 🟡 **The https dial sent SNI `localhost` and `Host: localhost` to a
    peer dialled by IP.** *(Found and fixed 2026-09-23.)* No curl or browser
    sends an SNI for an IP; ours did, a constant string on every such dial.
    Now no SNI and `Host: <ip>[:port]`, as curl; measured equal to curl's
    ClientHello dialling the same IP (`windows-wine-test.sh`). The quic dial
    already sent none.
  - [x] 🟡 **The Windows GUI wrote `HttpsFront` / `HttpsFrontPort`, keys no
    core reads.** *(Found and fixed 2026-09-23.)* The setting looked saved and
    did nothing; the core's key is `HttpsPort` (`0` = off). `backend/transports.py`,
    the panel and the example now use `HttpsPort`, and `QuicPort`/`HttpsPort`
    accept 0; GUI tests 86 passed, 1 skipped (`tincstack/winpytest:dev`).
  - [x] 🟡 **`%zd` / `%zu` in log formats on the Windows build.** *(Fixed
    2026-09-23 with the Windows TLS carriers: every `%z` in the core is a cast
    and `%ld`/`%lu` -- `transport.c`, `transport_sf.c`, `obfs.c`, `httpc.c`,
    `pool.c`; the mingw build has no format warnings left.)* The mingw
  ...
  - 🟡 **`httpc` `dechunk()` trusts the chunk size: heap overflow.** A chunk
    ...
    higher. Fix: `if(!chunk || chunk > len - in) break;`. Since 2026-09-23
    `httpc` is compiled into the Windows `tinc.exe` too; there `strtoul` is
    32-bit (LLP64), so `in + chunk` cannot wrap a 64-bit `size_t` -- by
    reading, not run. Still open.
    **Fixed 2026-09-26 (stream C):** `chunk > len - in` checked, the size
    parsed with overflow protection (`test_chunk_size_cannot_wrap` under
    ASan/UBSan). The `fuzz_dechunk` harness this note names is not in the
    repository, so it was not re-run.
  ...
  - ~~🟠 **The decoy is fingerprintable.**~~ **Resolved 2026-09-23:** the
    decoy is nginx's, see "The decoy (active), confirmed" under the owner's
    requirement. *(raised from 🟡 on 2026-09-23 under
...
- ~~🟠 **`tls.c` needs OpenSSL 3.0, so the documented Android `openssl` build does
  not compile.**~~ **Fixed 2026-09-24:** `build-core.sh` builds OpenSSL 3.5.7
  (libssl + libcrypto) instead of LibreSSL; the default build compiles and
  the APK has https/quic (M7/Android item under the TLS ClientHello work). Found by stream Y, verified in the consolidated tree:
...
- ~~🟠 **The core cannot write keys where `tmpfile(3)` has nowhere to go.**~~
  **Fixed 2026-09-24** (`yamlconf.c`): when `tmpfile()` fails the temporary is
  a `mkstemp` file (0600, unlinked at once) next to the config. Measured: a
  container with a read-only `/tmp` -- the previous core `Could not open
  configuration file /c/tinc.yaml: Read-only file system`, rc 1; this one
  reads it, no file left behind. On the emulator `tinc set` run as the app
  without `TMPDIR` now works (`join-on-emulator.sh` TRANSPORT step).

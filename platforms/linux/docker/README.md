...
| `LOG_LEVEL` | `tincd -d` | `1` |
| `CERT_RENEW` | `1`: run `tinc cert renew` on a timer when `CertDomain` and `CloudflareToken` are set. Off by default until peers can follow a renewed certificate: a renewal changes the fingerprint they pin (PLAN.md Known Issues) | `0` |

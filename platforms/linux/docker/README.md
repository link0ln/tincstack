...
| `LOG_LEVEL` | `tincd -d` | `1` |
| `CERT_RENEW` | `1`: run `tinc cert renew` on a timer when `CertDomain` and `CloudflareToken` are set. Peers follow a renewed certificate on their own (they re-pin it after SPTPS authenticates this node). Off by default | `0` |

	...
	*/
	if(sleep_time > 2 * udp_discovery_timeout) {
		...
		*/
		...
		/* On NAT'd nodes the external UDP mapping for our fixed source port is
		   very likely stale or filtered after a long sleep. Rebind UDP sockets
		   to fresh ephemeral ports so the post-wake reconnection hole-punches
		   from clean mappings instead of getting stuck on the old ones. */
		if(udp_rebind_on_wake) {
			rebind_udp_sockets();
		}
	}

...
static void load_one_node(const char *name) {
	if(!check_id(name)) {
		return;
	}

	node_t *n = lookup_node((char *) name);

	splay_tree_t config;
	init_configuration(&config);
	read_config_options(&config, name);
	read_host_config(&config, name, true);

	if(!n) {
		n = new_node((char *) name);
		node_add(n);
	}

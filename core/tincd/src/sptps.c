...
static bool sptps_receive_data_datagram(sptps_t *s, const uint8_t *data, size_t len) {
	...
	if(!s->instate) {
		if(seqno != s->inseqno) {
			/* Stale data packets from a previous SPTPS session can arrive after
			 * one peer restarted its tincd but the other peer still has packets
			 * draining from its UDP send buffer / NAT queue. Those carry the
			 * previous session's high seqno (e.g. several thousand) while we
			 * just reset and expect a small handshake seqno. Treat them as
			 * stale and silently drop instead of escalating: returning an
			 * error here causes the caller to call send_req_key() which would
			 * tear down the freshly started handshake again. Repeated resets
			 * livelock recovery because stale packets keep arriving for many
			 * seconds. Heuristic: if seqno is sharply higher than expected,
			 * it's almost certainly stale, not a genuine handshake out-of-order.
			 */
			if(seqno > s->inseqno + 64) {
				return false;
			}
